...
- **阶段 2：分页和页面指示器**：进行中（CollectionView 风格分页 + 拖拽跟随 + 滚轮翻页 + 边缘弹簧效果 + 指示器固定在底部）。
- **阶段 3：搜索与过滤**：规划中。
...
- **`ContentView.swift` / `LaunchpadView.swift`**：
  - SwiftUI 主界面：`ContentView` 提供背景与退出手势，`LaunchpadView` 负责图标网格展示（当前实现了 CollectionView 风格的分页网格）。
- **`SwipePagingCaptureView.swift`**：
  - 捕获鼠标拖拽手势，实现类似 iOS CollectionView 的拖拽翻页效果。
...
- **1.2 单页图标网格布局（进行中）**
  ...
  - 处理图标尺寸、行列间距、行数/列数配置。
  - 支持可配置的图标大小和字体大小（已实现，实时生效，通过 `LaunchpadGridSettings` 持久化）。
  - 支持简单的 hover/点击高亮效果（规划中）。
  - **CollectionView 风格分页**（已实现）：
    - 同时渲染所有页面，使用 HStack 水平排列
    - 通过 offset 控制显示位置，实现平滑的滑动效果
    - 拖拽和滚轮翻页都有平滑的动画过渡
  - **网格布局优化**：
...
- **2.2 分页切换交互（进行中）**
  ...
  - 支持鼠标滚轮 / 触控板上下滑动切换页（已通过全局 scroll 事件监听实现，带平滑滑动动画）。
  - 支持鼠标拖拽切换页（已实现，类似 iOS CollectionView 效果）：
    - 拖拽时网格跟随移动，能看到相邻页面
    - 松手时根据拖拽距离（≥80pt）和速度（≥400pt/s）决定是否翻页
    - 翻页时有平滑的滑动动画，复位时有弹性回弹效果
  - 边缘页面弹簧效果：在第一页/最后一页继续滚动时，会有小的偏移然后回弹，提供视觉反馈。
- **2.3 页面指示器 UI（进行中）**
  - 底部小圆点指示当前页（当前已有基础实现）。
...
- **当前支持的设置项（实时生效）**：
  ...
  - 上下边距：通过 `Slider` 控制 `verticalMargin`（范围：80-260，默认：160）。
  - 图标大小：通过 `Slider` 控制 `iconSize`（范围：32-128，默认：108）。
  - 字体大小：通过 `Slider` 控制 `appNameFontSize`（范围：8-20，默认：13）。
...
- **关于屏幕尺寸适配的思考（待实现）**
  - **当前实现**：使用固定像素值（points）作为边距、图标大小、字体大小等参数。macOS 的 Retina 缩放会自动处理 1x/2x 缩放，但在不同物理尺寸的屏幕上（如 13 寸笔记本 vs 27 寸外接显示器），相同的像素值会带来不同的视觉密度和可用空间。
  - **是否需要百分比适配**：
    - **短期（当前）**：固定值可用，适合单屏或固定使用场景。
    - **中期（推荐）**：引入基于屏幕尺寸的百分比/比例适配：
      - 边距：可用屏幕宽高的百分比（例如：左右边距 = 屏幕宽度 × 0.15）。
      - 图标大小：基于屏幕宽度或高度计算（例如：`min(screenWidth, screenHeight) * 0.08`）。
      - 字体大小：随图标大小按比例缩放（例如：`iconSize * 0.12`）。
    - **长期**：提供“固定值模式”和“自适应模式”两种模式，让用户选择。
  - **实现建议**：
    - 在 `LaunchpadGridSettings` 中增加一个“自适应模式”开关。
    - 开启时，读取 `NSScreen.main?.frame` 获取屏幕尺寸，边距、图标大小等按屏幕尺寸的百分比计算。
    - 用户仍可在设置中微调这些百分比，或切换回固定值模式。
